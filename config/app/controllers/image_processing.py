...
class ImageProcessingController(ControllerBase):
    ...
    @http_post('/process')
    def process_images(self, request: HttpRequest, images: Optional[List[UploadedFile]] = File(None), data: Optional[Base64Format] = None):
        try:
            ...
            try:
                payload = JsonPayload.objects.create(json_payload=temp if format=='f' else data)
